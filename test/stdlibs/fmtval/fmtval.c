...
char *fmtval(char *buf, double d, int fdarg)
...
int main() {
	...
	int nitems = 1340000;
	double size = 1204787;
	double cost = 18278.85;
	char buf [100];
	printf("You ordered %s sheets,", fmtval(buf, (double) nitems, FV_INTEGER));
	printf( " each %s square cm.\n", fmtval(buf, size, 3));
	printf("P1ease remit %s to our New York office,\n", fmtval (buf, cost, FV_INT_CUR));
	printf ( "(that's %s).\n", fmtval(buf, cost, FV_LCL_CUR));	
	return 0;
}
