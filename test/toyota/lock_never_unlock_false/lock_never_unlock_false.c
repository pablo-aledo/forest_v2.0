...
void *lock_never_unlock_004_tsk_001(void *input)
{
   ...
   if (ip < 0)
   pthread_mutex_unlock( &lock_never_unlock_004_glb_mutex_1 );/*Tool should detect this line as error*/ /* ERROR:Lock Never Unlock */
#endif /* defined(CHECKER_POLYSPACE) */
...
void * lock_never_unlock_006_tsk_001(void *input)
{
   ...
   if (ip < 0)	/*Tool should detect this line as error*/ /* ERROR:Lock Never Unlock */
	...
	while (i>0)
	{
		   if (ip >= 0)
		   {
			   ...
			   if(i!=5)
			   {
				   ...
				   pthread_mutex_unlock( &lock_never_unlock_007_glb_mutex_2 );/*Tool should detect this line as error*/ /* ERROR:Lock Never Unlock */
			   }
		   }
		   i--;
	}
...
void * lock_never_unlock_008_tsk_001(void *input)
{
	...
	for (i=0;i<=4;i++)
	{
		if (ip >= 0)
		{
			...
			if(i!=4)
			   pthread_mutex_unlock( &lock_never_unlock_008_glb_mutex_1 );/*Tool should detect this line as error*/ /* ERROR:Lock Never Unlock */
		}
	}
#if defined PRINT_DEBUG
     ...
     do
	{
		   if (ip >= 0)
		   {
			   ...
			   if(i!=5)
			   {
				   ...
				   pthread_mutex_unlock( &lock_never_unlock_009_glb_mutex_2 );/*Tool should detect this line as error*/ /* ERROR:Lock Never Unlock */
			   }
		   }
		   i--;
	}	while (i>0);
